package smu.db_project.domain.auth.controller;

import lombok.RequiredArgsConstructor;
import org.springframework.http.ResponseEntity;
import org.springframework.web.bind.annotation.*;
import smu.db_project.domain.auth.dto.*;
import smu.db_project.domain.auth.service.AuthService;
...
public class AuthController {
    ...
    @PatchMapping("/{sId}")
    public ResponseEntity<AuthResponseDto> updateStudentInfo(
            @PathVariable String sId,
            @RequestBody UpdateStudentRequestDto request) {

        AuthResponseDto response = authService.updateStudentInfo(sId, request);
        return ResponseEntity.ok(response);
    }
}