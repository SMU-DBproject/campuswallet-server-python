package smu.db_project.domain.category.service;
...
public class CategoryService {
    ...
    private void init() {
        ...
        getMaxSpendingCategoryCall = new SimpleJdbcCall(dataSource)
                ...
                .declareParameters(
                        ...
                        new SqlOutParameter("p_month", Types.VARCHAR), // 🆕 추가
                        new SqlOutParameter("p_success", Types.VARCHAR),
                        new SqlOutParameter("p_message", Types.VARCHAR)
                );
    }
    ...
    public MaxSpendingCategoryDto getMaxSpendingCategory(Long studentId) {
        try {
            ...
            if (!"Y".equals(success)) {
                throw new RuntimeException("최대 소비 조회 실패: " + message);
            }

            String category = (String) out.get("p_category");
            Number total = (Number) out.get("p_total");
            String month = (String) out.get("p_month"); // 🆕

            return new MaxSpendingCategoryDto(
                    category,
